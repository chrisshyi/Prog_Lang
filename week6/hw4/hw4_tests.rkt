...
(define nats
  (letrec ([f (lambda (x) (cons x (lambda () (f (+ x 1)))))])
    (lambda () (f 1)))) ; stream of natural numbers
...
(define tests
  (test-suite
   ...
   (check-equal? (stream-for-n-steps nats 7) (list 1 2 3 4 5 6 7) "stream-for-n-steps test")
   ...
   (check-equal? (stream-for-n-steps (stream-add-zero nats) 2) (list (cons 0 1) (cons 0 2)))
   (check-equal? (stream-for-n-steps (stream-add-zero nats) 5) (list (cons 0 1) (cons 0 2) (cons 0 3) (cons 0 4) (cons 0 5)))
   ...
   (check-equal? (vector-assoc 5 (vector (cons 2 1) (cons 3 1) 10 (cons 4 1) (cons 5 1))) (cons 5 1) "vector-assoc test")
   
   
   ; cached-assoc tests
   (check-equal? ((cached-assoc (list (cons 1 2) (cons 3 4)) 3) 3) (cons 3 4) "cached-assoc test")
   (check-equal? ((cached-assoc (list (cons 1 2) (cons 3 4)) 1) 5) #f "cached-assoc test")
