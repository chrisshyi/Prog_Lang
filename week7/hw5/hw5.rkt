...
(define (eval-under-env e env)
  (let ([is-value-type (lambda (e) (or (int? e) (closure? e) (aunit? e)))])
  (cond [(var? e) 
         ...
         (let ([v1 (eval-under-env (call-funexp e) env)]
           ...
           (if (closure? v1)
               (let ([func (closure-fun v1)])
                 (if (fun-nameopt func) ; function's name is not #f
                     (let ([extended-env (append (list (cons (fun-nameopt func) v1)
                                               (cons (fun-formal func) v2))
                                               (closure-env v1))])
                       (eval-under-env (fun-body func) extended-env))
                     (let ([extended-env (append (list (cons (fun-formal func) v2)) (closure-env v1))])
                       (eval-under-env (fun-body func) extended-env))))
               (error "Not calling a closure")))]
        [(fst? e)
         (let ([v (eval-under-env (fst-e e) env)])
           (if (apair? v)
               (apair-e1 v)
               (error "not called on a pair!")))]
        [(snd? e)
...
(define mupl-mapAddN 
  (mlet "map" mupl-map
        (fun #f "num" (call (var "map") (fun #f "x" (add (var "num") (var "x")))))))
